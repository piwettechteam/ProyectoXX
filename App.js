...
import { StyleSheet, Text, View, Image, ScrollView,Button,FlatList,TouchableOpacity,AsyncStorage,LayoutAnimation} from 'react-native';
...
// Function for register For PushNotifications
  ...
  _onDone = async ()  => {
    ...
    this.makeTheDrawer(this.state.data)
  ...
  render() {
    if(this.state.metaLoaded&&this.state.isReady){
      ...
      if(this.state.meta.config.loginRequired&&!this.state.userAuthenticated){
        ...
        return (<this.loginNavi isLoggedIn={this.state.userAuthenticated} isReqUserVarification={this.state.meta.config.userVarification} />);
      }else{
